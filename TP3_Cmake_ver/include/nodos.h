...
class configuracion
{
	...
	public:
		...
		void setPausasImpresion(bool pausas) { pausaEntreImpresion = pausas; }
		//no le puse constructor pq soy un ciruja
		bool permitePromedio() const { return this->calcPromedio; }
		bool muestraPosicion() const { return this->mostrarPosicion; }
		bool permitePausas() const { return this->pausaEntreImpresion; }
};
...
class Lista
{
	private:
		Nodo* cabeza = nullptr;
		bool agregarCuandoNulo(Nodo* nodo);
	...
	public:
		...
		bool agregarPosicion(std::string& nombre, std::vector<int>& valores, int pos);
		int solicitarPosicion(Nodo* nodo);
		void imprimirLista(const configuracion& Config);
		void limpiarResto();
